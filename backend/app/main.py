...
class IssuerCurveResult(BaseModel):
    ...
    slope_total: Optional[float]
    yield_1y: Optional[float]
    yield_3y: Optional[float]
...
class UpdateRequest(BaseModel):
    """更新请求"""
    date: Optional[str] = None  # 不指定则更新最新交易日
...
@app.get("/api/dates", summary="获取可用日期列表")
...
@app.get("/api/analysis/{date}", response_model=DateAnalysisResult, summary="获取分析结果")
async def get_analysis(date: str):
    ...
    try:
        ...
        return DateAnalysisResult(
            date=date,
            normal=[IssuerCurveResult(**item) for item in data.get("normal", [])],
            perpetual=[IssuerCurveResult(**item for item in data.get("perpetual", [])]
        )
...
@app.post("/api/update", response_model=UpdateResponse, summary="更新数据")
async def update_data(request: UpdateRequest):
    ...
    try:
        ...
        return UpdateResponse(success=False, message="更新超时，请稍后查看结果")
    except Exception as e:
        logger.error(f"更新数据失败: {e}")
        return UpdateResponse(success=False, message=str(e))


@app.get("/api/export/{date}", summary="导出Excel")
